...
async function createMarketTest() {
    ...
    try {
        ...
        let enterLoginPage = await driver.wait(until.elementLocated(By.linkText("Logowanie")));
        enterLoginPage.click()

        let startTypingLogin = await driver.wait(until.elementLocated(By.name("username")));
        startTypingLogin.sendKeys("moderator");
        await driver.findElement(By.name("password")).sendKeys("moderator123");
        await driver.findElement(By.id("loginButton")).click();

        let enterCreateMarketForm = await driver.wait(until.elementLocated(By.id("addNewMarket")));
        enterCreateMarketForm.click();

        let startTypingMarketTopic = await driver.wait(until.elementLocated(By.name("topic")));
        ...
        let enterBetTitle = await driver.wait(until.elementLocated(By.name("title")));
        ...
        let confirmBets = await driver.wait(until.elementLocated(By.id("confirmBetsButton")));
        confirmBets.click();
...
async function changePassword() {
    ...
    try {
        ...
        let enterChangePasswordForm = await driver.wait(until.elementLocated(By.id("openEditPasswordButton")));
        enterChangePasswordForm.click();

        let startTypingOldPassword = await driver.wait(until.elementLocated(By.name("oldPassword")));
        startTypingOldPassword.sendKeys("wojtek1234");

       await driver.findElement(By.name("newPassword")).sendKeys("wojtek12345");
       await driver.findElement(By.name("repeatedNewPassword")).sendKeys("wojtek12345");

       await driver.findElement(By.id("submitNewPassword")).click();

       let loginWithNewPassword = await driver.wait(until.elementLocated(By.name("username")));
